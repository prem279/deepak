...
import com.lmig.ci.lmbc.empr.muw.account.config.EurekaClientConfig;
import com.lmig.ci.lmbc.empr.muw.account.config.MuwAccountRepoConfig;
...
@ResourceBasedKey(profile = { EnvironmentConstants.JUNIT,
		EnvironmentConstants.LOCAL }, location = "classpath:key/muwacct-local-keys.key")
@SpringBootApplication(exclude = EndpointMBeanExportAutoConfiguration.class)
@EnableResourceServer
@ComponentScan(basePackages = { "com.lmig.ci.lmbc.empr.auth.util", "com.lmig.ci.lmbc.empr.muw.account" })
@EnableSwagger2
@Import({ springfox.documentation.spring.data.rest.configuration.SpringDataRestConfiguration.class,
		MuwAccountRepoConfig.class })

public class MuwAccountService {

	private static final Logger logger = LoggerFactory.getLogger(MuwAccountService.class);

	public static void main(String[] args) {

		try {

			SpringApplication.run(MuwAccountService.class, args);
		} catch (Exception e) {
			e.printStackTrace();
		}

		System.out.println("Starting Spring Boot Application: MUW Account Service");
	}
